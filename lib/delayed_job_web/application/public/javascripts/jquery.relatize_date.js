...
(function($) {
  ...
  $.extend($.relatizeDate, {
    ...
    distanceOfTimeInWords: function(fromTime, toTime, includeTime) {
      ...
      var days_from_now = parseInt(Math.abs(delta / 86400)).toString();
      var hours_from_now = parseInt(Math.abs(delta / 3600)).toString();
      var minutes_from_now = parseInt(Math.abs(delta / 60)).toString();
      var seconds_from_now = parseInt(Math.abs(delta)).toString();

      var days_ago = parseInt(delta / 86400).toString();
      var hours_ago = parseInt(delta / 3600).toString();
      var minutes_ago = parseInt(delta / 60).toString();
      var seconds_ago = parseInt(delta).toString();

      if (delta < 0) {
        return days_from_now + " days - " + hours_from_now + " hours - " + minutes_from_now + " minutes - " + seconds_from_now " seconds from now";
      } else {
        return days_ago + " days - " + hours_ago + " hours - " + minutes_ago + " minutes - " + seconds_ago " seconds ago";
      }
    }
  })
})(jQuery);