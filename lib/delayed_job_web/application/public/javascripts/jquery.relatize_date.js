...
(function($) {
  ...
  $.extend($.relatizeDate, {
    ...
    distanceOfTimeInWords: function(fromTime, toTime, includeTime) {
      ...
      var seconds_from_now = parseInt(Math.abs(delta % 60));
      var minutes_from_now = parseInt(Math.abs((delta / 60) % 60));
      var hours_from_now = parseInt(Math.abs((delta / (60 * 60)) % 24));
      var days_from_now = parseInt(Math.abs(delta / (60 * 60 * 24)));

      return days_from_now.toString() + " days - " + hours_from_now.toString() + " hours - " + minutes_from_now.toString() + " minutes - " + seconds_from_now.toString() + " seconds from now";
    }
  })
})(jQuery);
